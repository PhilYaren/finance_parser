...
impl Display for ReadError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ReadError::Txt { line, message } => match line {
                Some(line) => write!(f, "Text read error occured at line {line}: {message}"),
...
impl Display for WriteError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            WriteError::Txt { line, message } => {
                if let Some(l) = line {
                    write!(f, "Text write error occured at line {l}: {message}")
                } else {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match *self {
            FileError::ExtensionError(ref extension) => {
                write!(f, "Unsuported extension: {extension}")
            }
            FileError::NonExistentExtension => write!(
                f,
                "File extension is not provided - please write file in format [exaple_name].[ext]"
            ),
